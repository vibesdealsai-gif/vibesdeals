...
function renderContent(content: string) {
  ...
  const flushList = () => {
    ...
    elements.push(
      listType === 'ul' ? (
        <ul key={`ul-${elements.length}`} className="list-disc pl-6 mb-6 space-y-2">
        ...
        <ol key={`ol-${elements.length}`} className="list-decimal pl-6 mb-6 space-y-2">
          {items.map((item, index) => (
            <li key={index} className="leading-7">
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed === '---') {
      flushList();
      elements.push(
        <hr key={`hr-${index}`} className="my-8 border-gray-200" />
      );
      return;
    }
