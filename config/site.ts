export const siteConfig = {
  name: "Vibes Deals",
  tagline: "Discover Great Deals. Shop Smarter.",
  description: "Discover trending products, amazing offers, and carefully selected deals from popular online stores.",
  contactEmail: "your-email@example.com",
  social: {
    instagram: "https://instagram.com/@prabhudevaarts",
    twitter: "https://x.com/@prabhudeva_750",
    facebook: "https://facebook.com/@prabhudevaarts",
    pinterest: "https://pinterest.com/@prabhudevaarts",
    linkedin: "https://linkedin.com/in/prabhudevaarts"
  },
  links: {
    home: "/",
    deals: "/search?sort=discount",
    bestSellers: "/search?sort=bestseller",
    categories: "/#categories",
    blog: "/blog",
    about: "/about",
    contact: "/contact",
    privacy: "/privacy",
    terms: "/terms",
    affiliateDisclosure: "/affiliate-disclosure",
    disclaimer: "/disclaimer"
  },
  affiliateDisclosure: "As an affiliate website, Vibes Deals may earn a commission from qualifying purchases made through links on this website."
};
